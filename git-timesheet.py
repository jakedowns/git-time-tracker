...
# Initialize previous commit time
...
while start_datetime <= end_datetime:
    ...
    for line in output.strip().split('\n'):
        ...
        if previous_commit_time:
            gap = commit_time - previous_commit_time
            gap_hours = gap.total_seconds() / 3600

            # If the gap is greater than 1 hour, update total hours
            if gap_hours > 1:
                total_estimated_hours += round(gap_hours)
